...
router.get('/', async (req, res) => {
	const shirtsRef = db.collection('shirts')
	const snapshot = await shirtsRef.get()
...
router.post('/', async (req, res) => {
	...
	const docRef = await db.collection('shirts').add(object)
	res.send(docRef.id)
})
...
router.put('/:id', async (req, res) => {
	...
	const docRef = db.collection('shirts').doc(id)
